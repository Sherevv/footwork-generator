...
export default class NavbarComponent extends Vue {
    ...
    created() {
        ...
        if (lang) {
            if (lang != this.$translate.lang()) {
                if (this.$translate.langs.indexOf(lang) !== -1) { // change lang
                    this.$translate.setLang(lang.toString());
                    this.$ls.set('lang', lang);
                } else { // get lang from LocalStorage
    ...
    changeLanguage(lang: string) {
        ...
        this.$router.replace({params: {lang: lang}, query: this.$route.query}).catch(error => {
    ...
    trySetLangFromLocalStorage(): void {
        ...
        if (lang && lang != this.$translate.lang() && this.$translate.langs.indexOf(lang) !== -1) { // change lang
            this.$translate.setLang(lang.toString());
        } else { //  use current lang
